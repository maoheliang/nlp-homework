...
def use_ai_get_mid_output(full_text, client: Client, client_config: dict):
    ...
    prompt = (
        ...
        "5. **存在问题**：同时存在**总结**和**具体问题**。**总结**需要你作为专家总结简短的一句话,20个字以内；**具体问题**，使用文档中明确存在的信息分点回答。\n"
        "6: **原因分析**：如果 **存在问题** 不为空，则请作为专家结合文档从多方面给出自己的分析，**格式必须严格参照下面JSON模板**，并确保回答涵盖至少3个方面，每个方面下有1-3点具体分析意见。每个方面必须分点列出，并简洁明确。\n"
        "7. **格式规范**：\n"
